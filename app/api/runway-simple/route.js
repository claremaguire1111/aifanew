...
// Simple endpoint to directly call the Runway API with correct parameters
export async function POST(req) {
  console.log('POST request received to runway-simple endpoint');
  ...
  try {
    ...
    console.log('Received payload with structure:', Object.keys(payload));
    
    // Make direct request to the development API URL as specified in docs
    const response = await fetch('https://api.dev.runwayml.com/v1/image_to_video', {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${runwayApiKey}`,
        'Content-Type': 'application/json',
        'X-Runway-Version': '2024-11-06' // Required according to docs
      },
      // Format according to the official API docs - flat structure with version 2024-11-06
      body: JSON.stringify({
        model: payload.model,
        promptImage: payload.input.promptImage,
        promptText: payload.input.promptText,
